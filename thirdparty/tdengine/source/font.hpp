...
typedef struct {
	dn_fixed_array<dn_font_t> font_infos;
	dn_fixed_array<dn_glyph_t> glyph_infos;
	dn_fixed_array<Vector2> vertex_data;
	dn_fixed_array<Vector2> uv_data;
} dn_fonts_t;
dn_fonts_t dn_fonts;

DN_IMP void       dn_font_init();
DN_API void       dn_font_create(dn_font_descriptor_t descriptor);
DN_API dn_font_t* dn_font_default();
DN_API dn_font_t* dn_font_find(const char* name);
DN_API dn_font_t* dn_font_find_hash(size_t hash);
...
void dn_font_init() {
	dn_fixed_array_init_t(&dn_fonts.vertex_data, 1024);
...
dn_font_t* dn_font_default() {
	return NULL;
}

void dn_font_add_to_imgui(const char* id) {
	auto font = dn_font_find(id);
	if (!font) {
...
void dn_font_create(const char* id, const char* file_path, u32 size) {
	...
	dn_font_t* font = arr_push(&font_infos);
...
dn_font_t* dn_font_find_default() {
	return dn_font_find("default");
}

dn_font_t* dn_font_find_hash(size_t hash) {
	arr_for(font_infos, font) {
		if (font->hash == hash) return font;
	}
	return nullptr;
}
dn_font_t* dn_font_find(const char* id) {
