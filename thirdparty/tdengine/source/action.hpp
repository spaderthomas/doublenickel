...
struct SteamInputManager {
	...
	dn_tstring_t get_controller_name(Steam::Controller controller);
...
DN_API void register_action(const char* name, u32 key, u32 key_even, const char* action_set);
DN_API void register_action_set(const char* name);
DN_API void activate_action_set(const char* name);
