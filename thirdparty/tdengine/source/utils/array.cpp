...
//////////////////
template<typename T>
dn_array_tMarker<T> dn_array_marker_make(dn_array_t<T>* array) {
	dn_array_tMarker<T> marker;
	dn_array_marker_init(&marker, array);
	return marker;
}

template<typename T>
void dn_array_marker_init(dn_array_tMarker<T>* marker, dn_array_t<T>* array) {
	marker->begin = array->size;
	marker->array = array;
}

template<typename T>
void dn_array_marker_freeze(dn_array_tMarker<T>* marker) {
	assert(marker->array);
	marker->frozen_size = marker->array->size;
}

template<typename T>
i32 dn_array_marker_count(dn_array_tMarker<T>* marker) {
