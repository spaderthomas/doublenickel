...
local TextEditor = dn.editor.define('TextEditor')
...
function TextEditor:init(params)
  params = params or {}
  self.size = params.size or dn.vec2(0, 0)
  ...
  self.selection_color = dn.colors.spring_green:to_u32()
  ...
  self.character_size = dn.vec2(7.63, 20)
...
function TextEditor:update()
  ...
  dn.editor.begin_window('Dialogue Node Text')

  self.frame = self.frame + 1
  self.time_since_click = self.time_since_click + dn.dt
  self.last_action = nil
  ...
  local focus = dn.editor.is_window_focused()
  local hover = dn.editor.is_window_hovered()

  if self.state == editor_state.idle then
    for code = glfw.keys.SPACE, glfw.keys.GRAVE_ACCENT, 1 do
      self:handle_check_repeat(dn.dt, code, self.handle_alpha, self, string.char(code))
      if code == glfw.keys.GRAVE_ACCENT then break end
    end

    self:handle_check_repeat(dn.dt, glfw.keys.BACKSPACE, self.handle_backspace, self)
    self:handle_check_repeat(dn.dt, glfw.keys.LEFT, self.handle_left_arrow, self)
    self:handle_check_repeat(dn.dt, glfw.keys.RIGHT, self.handle_right_arrow, self)
    self:handle_check_repeat(dn.dt, glfw.keys.DOWN, self.handle_down_arrow, self)
    self:handle_check_repeat(dn.dt, glfw.keys.UP, self.handle_up_arrow, self)
  ...
  elseif self.state == editor_state.dragging then
    if focus or hover then
      self.selection_end = screen_to_point(self, dn.vec2(imgui.GetMousePos()))
    end
...
function handle_click(editor)
  if imgui.IsMouseClicked(0) then
    editor.point = screen_to_point(editor, dn.vec2(imgui.GetMousePos()))
...
function point_to_screen(editor, point)
  local screen = dn.vec2(imgui.GetCursorScreenPos())
...
function screen_to_point(editor, screen)
  ...
  local screen_begin = dn.vec2(imgui.GetCursorScreenPos())
...
function show_grid(editor)
  local line_color = dn.color32(255, 40, 200, 100)
  local canvas = dn.vec2(imgui.GetCursorScreenPos())
  local window = dn.vec2(imgui.GetWindowSize())
  for x = 0, window.x, editor.character_size.x do
    local top = dn.vec2(x, 0):add(canvas)
    local bottom = dn.vec2(x, window.y):add(canvas)
    imgui.GetWindowDrawList():AddLine(top.x, top.y, bottom.x, bottom.y, line_color)
  end
  for y = 0, window.y, editor.character_size.y do
    local top = dn.vec2(0, y):add(canvas)
    local bottom = dn.vec2(window.x, y):add(canvas)
    imgui.GetWindowDrawList():AddLine(top.x, top.y, bottom.x, bottom.y, line_color)
  end
end
