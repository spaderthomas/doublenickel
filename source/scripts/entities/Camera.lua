Camera = doublenickel.entity.define('Camera')
...
Camera.states = doublenickel.enum.define(
  'CameraState',
  {
    Idle = 0,
    Interpolate = 1,
...
function Camera:init(params)
  ...
  self.noise_offset = doublenickel.vec2()


  self.enabled = true
  self.default_state = doublenickel.enum.load(params.default_state) or doublenickel.enums.CameraState.Idle
  self.state = self.default_state

  self.zoom = 1.0
  self.zoom_state = doublenickel.enums.CameraZoomState.Idle
end
...
function Camera:on_main_menu()
  self.offset = doublenickel.vec2(0, -1080)
  self.noise_offset = doublenickel.vec2()
end
...
function Camera:update_offset()
  if self.state == doublenickel.enums.CameraState.Idle then

  elseif self.state == doublenickel.enums.CameraState.Interpolate then
    self:update_interpolation()
  elseif self.state == doublenickel.enums.CameraState.InterpolateToPlayer then
    ...
    -- self:update_interpolation()
  elseif self.state == doublenickel.enums.CameraState.FollowPlayer then
    -- self:move_to_player()
...
function Camera:update_interpolation()
  ...
  if self.interpolation:is_done() then
    if self.interpolation.stop_after_interpolate then
      self.state = doublenickel.enums.CameraState.Idle
    else
      self.state = self.default_state
    end
...
function Camera:update_zoom()
  if self.zoom_state == doublenickel.enums.CameraZoomState.Idle then

  elseif self.zoom_state == doublenickel.enums.CameraZoomState.Interpolate then
...
function Camera:update_sway()
  if self.perlin then
    ...
    self.noise_offset.x = doublenickel.math.lerp(0, nx, blend)
    self.noise_offset.y = doublenickel.math.lerp(0, ny, blend)
...
function Camera:move_to_player()
  ...
  -- local player = doublenickel.find_entity('Player')
  -- if not player then return end

  -- local collider = player:find_component('Collider')

  -- local position = collider:get_position()
  -- local nx, ny = doublenickel.dn_window_get_native_resolution()
...
function Camera:clamp_to_bounds(offset)
  ...
  local bounds = doublenickel.find_entity('CameraBounds')
  if bounds and doublenickel.tick then
    if bounds.lock then
...
function Camera:interpolate_to(position, interpolation)
  self.state = doublenickel.enums.CameraState.Interpolate
...
--   interpolation = interpolation or doublenickel.interpolation.SmoothDamp2:new()
--   self:interpolate_to(self:get_player_offset(2), interpolation)
--   self.state = doublenickel.enums.CameraState.InterpolateToPlayer
-- end
...
function Camera:begin_sway(amplitude, speed, time)
  ...
  self.perlin = {
    ...
    interpolation = doublenickel.interpolation.SmoothDamp:new({
      start = 0,
      target = 1,
      time = 1,
      exponent = 2,
...
function Camera:zoom_to(value, interpolation)
  interpolation = interpolation or doublenickel.interpolation.SmoothDamp:new()

  self.zoom_state = doublenickel.enums.CameraZoomState.Interpolate
...
function Camera:get_player_offset(n)
  ...
  -- local player = doublenickel.find_entity('Player'):find_component('Collider')
  -- local base_position = player:get_position()
  -- local nx, ny = doublenickel.dn_window_get_native_resolution();
  -- return doublenickel.vec2(base_position.x - nx / n, base_position.y - ny / n)
end
