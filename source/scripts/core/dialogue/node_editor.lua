...
NodeEditor = dn.class.define('NodeEditor')
...
function NodeEditor:init(params)
	...
	self.nodes = {}
	self.gnodes = {}
	self.camera = dn.vec2(0, 0)
	self.scroll_per_second = params.scroll_per_second or 3000
	self.window_position = dn.vec2(0, 0)
	self.window_size = dn.vec2(0, 0)
	self.node_padding = dn.vec2(8, 8)
	self.inter_node_padding = 25 -- If we automatically add a node, how far is it from the previous?
	self.canvas_rclick_pos = dn.vec2(0, 0)
	self.hovered = nil
	...
	self.link_style = {
		thickness = 3,
		color = dn.color32(200, 200, 200, 255),
		disconnect_color = dn.color32(255, 0, 0, 255),
	}
	self.zoom = 1
	self.speed = {
		zoom = .25,
	...
	self.colors = {
		background = dn.colors.rich_black:copy(),
		grid_line = dn.colors.white:alpha(.25),
		disconnect = dn.colors.cardinal:copy(),
		link = dn.colors.white:copy(),
	}
...
function NodeEditor:update(dt)
	...
	-- open
	if not dn.editor.is_window_focused('Game') then
...
function NodeEditor:update_input()
	self.window_size = dn.vec2(imgui.GetWindowSize()) 
	...
	self.input_cache.mouse = dn.vec2(imgui.GetMousePos())
...
function NodeEditor:update_state()
	...
	elseif self.state == editor_state.separate_box_drag then
		...
		local separator = self.separators[#self.separators]
		separator.br = dn.vec2(imgui.GetMousePos())
		self:do_multiselect()

		local color = dn.color32(200, 255, 200, 30)
...
function NodeEditor:draw_grid()
	...
	for x = grid.min.x, grid.max.x, grid.size do
		local top = self:world_to_zoom(dn.vec2(x, grid.min.y))
		local bottom = self:world_to_zoom(dn.vec2(x, grid.max.y))
	...
	for y = grid.min.y, grid.max.y, grid.size do
		local left = self:world_to_zoom(dn.vec2(grid.min.x, y))
		local right = self:world_to_zoom(dn.vec2(grid.max.x, y))
...
function NodeEditor:draw_nodes()
	for id, node in pairs(self.nodes) do
		...
		local contents_size = dn.vec2(imgui.GetItemRectSize())
		...
		local slot_color = self.on_node_color(id) or dn.colors.red:copy()
		...
		local color = self.on_node_color(id) or dn.colors.red:copy()
		...
		for index, input_id in pairs(children) do
			...
			if slot_distance > bezier_min_distance then
				local cp = dn.vec2(50 * self.zoom, 0)
	...
	if self.connecting then
		...
		local cursor = dn.vec2(imgui.GetMousePos())
...
function NodeEditor:push_scaled_font()
	...
	local clamped_font_size = dn.math.snap_to_range(base_font_size * self.zoom, range)
	imgui.PushFont(dn.editor.config.fonts.regular, clamped_font_size)
...
function NodeEditor:begin_canvas()
	local flags = 0
	flags = bitwise(dn.op_or, flags, ffi.C.ImGuiWindowFlags_NoScrollWithMouse)
	flags = bitwise(dn.op_or, flags, ffi.C.ImGuiWindowFlags_NoScrollbar)
	dn.editor.begin_window(self.name, flags)
	...
	local flags = bitwise(dn.op_or, flags, ffi.C.ImGuiWindowFlags_NoMove)

	dn.editor.begin_child('camera_region', 0, 0, flags)
...
function NodeEditor:update_translation()
	...
	-- @hack
	if dn.editor.is_window_focused('Text Editor') then return end
	if not dn.editor.is_window_hovered(self.name) then return end

	if self.input:down(glfw.keys.W) then
		delta.y = delta.y + (self.scroll_per_second * dn.dt)
	end
	if self.input:down(glfw.keys.S) then
		delta.y = delta.y - (self.scroll_per_second * dn.dt)
	end
	if self.input:down(glfw.keys.A) then
		delta.x = delta.x + (self.scroll_per_second * dn.dt)
	end
...
function NodeEditor:update_zoom()
	if self.input_cache.hovered and math.abs(self.input_cache.scroll.y) > 0 then
		...
		if math.abs(zoom_delta) > 0 then
			local hack = dn.vec2(18, 18):scale(1 / self.zoom)
...
function NodeEditor:create_node(kind, position)
	...
	self.gnodes[uuid] = {
		position = dn.vec2(position), -- Specified in world space
		size = dn.vec2(),
		pixel_size = dn.vec2()
	}
...
function NodeEditor:enter_multiselect_line_mode(sep_kind)
	...
	local separator = {
		begin = dn.vec2(imgui.GetMousePos()),
...
function NodeEditor:snap_to_node(id)
	local gnode = self.gnodes[id]
	local offset = dn.vec2(700, 400)
	self.camera = dn.vec2(gnode.position):scale(-1):add(offset)
end
