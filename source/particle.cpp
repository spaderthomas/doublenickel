...
bool ParticleSystem::spawn_particle() {
	...
	particle->lifetime = lifetime + dn_math_random_f32_slow(-1 * lifetime_jitter * lifetime, lifetime_jitter * lifetime);
	particle->accumulated = 0;
	
	if (position_mode == ParticlePositionMode::Bottom) {
		particle->position.x = dn_math_random_f32_slow(position.x, position.x + area.x);
	...
	if (jitter_base_velocity) {
		particle->velocity.start.x += dn_math_random_f32_slow(-1 * velocity_jitter.x, velocity_jitter.x);
		particle->velocity.start.y += dn_math_random_f32_slow(-1 * velocity_jitter.y, velocity_jitter.y);
	}
	if (jitter_max_velocity) {
		particle->velocity.target.x += dn_math_random_f32_slow(-1 * velocity_jitter.x, velocity_jitter.x);
		particle->velocity.target.y += dn_math_random_f32_slow(-1 * velocity_jitter.y, velocity_jitter.y);
	}
	...
	if (jitter_size) {
		auto jitter = dn_math_random_f32_slow(-1 * size_jitter, size_jitter);
		if (particle_kind == ParticleKind::Quad) {
	...
	if (jitter_opacity) {
		auto jitter = dn_math_random_f32_slow(-1 * opacity_jitter, opacity_jitter);
